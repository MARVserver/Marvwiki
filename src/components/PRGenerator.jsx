...
*Generated by MarvWiki PR Generator*`);
    ...
    const submitToDiscord = async () => {
        ...
        if (!WEBHOOK_URL || WEBHOOK_URL.includes('YOUR_WEBHOOK_ID')) {
            alert(lang === 'ja' ? 'Webhook URLが設定されていません。.envファイルを確認してください。' : 'Webhook URL is not configured. Please check your .env file.');
            return;
        }
        ...
        try {
            ...
            console.error(err);
            alert(lang === 'ja' ? '送信に失敗しました。Webhookの設定を確認してください。' : 'Failed to send. Please check Webhook settings.');
        }
    };
    ...
    const t = {
        ja: {
            title: '簡単PRジェネレーター',
            subtitle: '必要事項を入力するだけで、Wiki用とDiscord用の宣伝文を自動生成します。',
            type: '種類',
            nation: '国家',
            town: '町',
            name: '名称',
